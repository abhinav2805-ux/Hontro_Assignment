...
// Helper to write activity logs without breaking the main flow
...
router.get('/', async (req, res) => {
  try {
    ...
    const board = await Board.findOne({ _id: resolvedBoardId, userId: req.userId });
    if (!board) {
      return res.status(404).json({ message: 'Board not found.' });
    }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const { listId, position, title, description, priority, deadline, assignees } = req.body;
    ...
    if (req.io) {
      ...
      if (moved) {
        ...
        req.io.to(room).emit('task_moved', task);
      }
      // snake_case and camelCase for compatibility
      req.io.to(room).emit('task_updated', task);
      req.io.to(room).emit('taskUpdated', task);
    }
    res.json(task);
  } catch (err) {
    res.status(500).json({ message: 'Server error.', error: err.message });
